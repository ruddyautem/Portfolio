...
const Tabsbar = () => {
  ...
  useEffect(() => {
    const updateUnderlineStyle = () => {
      const activeIndex = navLinks.findIndex((link) => link.link === currentRoute);
      if (activeIndex !== -1 && tabsRef.current[activeIndex]) {
        const { left, width } = tabsRef.current[activeIndex].getBoundingClientRect();
        const containerLeft =
  ...
  return (
    <div className="bg-menu text-darker h-7">
      <div className="relative flex flex-row items-center justify-center lg:justify-start">
        ...
        <div
          className={`absolute ${activeStyles.pos} bg-accent h-px transition-all duration-300
            ease-out`}
          style={{
