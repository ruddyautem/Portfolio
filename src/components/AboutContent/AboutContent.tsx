...
const AboutContent = () => {
  ...
  const renderSkillsSection = (section, index) => {
    ...
    const animateClass = index === 0 ? 'item-animate-1' : index === 1 ? 'item-animate-2' : 'item-animate-3';

    return (
      <div
        key={section.title}
        className={`${animateClass} relative flex flex-col items-center gap-8 py-10 sm:py-14
